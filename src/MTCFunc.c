...
void SetDefaultConfiguration(DigitizerConfig_t *Dcfg) { //CAEN_DGTZ_DPP_PSD_Params_t *DPPParams
	...
	for(int ch=0; ch<Dcfg->Nch; ch++) {
		Dcfg->InputRange[ch] = InputRange_2Vpp; //_2Vpp _0_5Vpp
...
void InitHisto(Histograms_t *Histo, int RecordLength[MAX_CH], int N_CH){
	char str[100];
...
void DrawHisto(Histograms_t Histo, int N_CH){
	...
	if (Histo.fLayers){
		...
		for (int i = Histo.FirstToDraw; i<N_CH; i++)
			Histo.layers->Fill( i, Histo.fLayersCoeff == false ?  Histo.ampl[i]->GetMean( ) : ( Histo.ampl[i]->GetMean( ) * Histo.layers_k[i] + Histo.layers_b[i]));
		
		Histo.layers->GetYaxis()->SetTitle("Energy, MeV");
